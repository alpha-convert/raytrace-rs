...
use crate::{geom::{
    cube::Cube, intersectable::{Intersectable, Intersection}, interval::Interval, plane::Plane, quad::Quad, ray::Ray, sphere::Sphere, translation::Translation
...
fn construct_geom(gd : &GeomDesc, mat_map : &HashMap<String,Arc<dyn Material>>) -> Box<dyn Intersectable> {
    match gd {
                GeomDesc::Cube { c, r, mat } => {
                    let mat = mat_map.get(mat).expect(format!("material {} to be defined",mat).as_str()).clone();
                    Box::new(Cube::new(*c, *r, mat))
                },
                GeomDesc::Plane { center, normal, u_hat, v_hat, mat } => {
                    let mat = mat_map.get(mat).expect(format!("material {} to be defined",mat).as_str()).clone();
                    Box::new(Plane::new(*center, Unit::new_normalize(*normal), *u_hat, *v_hat, mat))
                },
